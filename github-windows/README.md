...
> **セキュリティ注意**: ラッパーは解決したトークンを PATH 上で最初に見つかった `github-mcp-server` に渡します。信頼できない実行ファイルが PATH の先頭に紛れているとトークンが渡る恐れがあるため、**PATH は信頼できるディレクトリのみで構成**してください（リポジトリローカルの実行ファイルを優先解決させない）。
