...
- PostToolUse は **非 write のステップでも発火**し、その回は `toolCall:null`（実測 stepIdx 1/4）。ハンドラは **`toolCall` が null か `name!="write_to_file"` なら即 no-op** とガードする。
- `${/}` の `Bad substitution`（#20）は今回の `hooks.json` に `${/}` を含めず**未再検証**。PWD 回避で不要。

帰結: **「保存した編集ファイルを対象に validator を回す」フック用途は 1.0.9 で初めて成立しうる**（編集ファイルパス＋自前バイナリ相対呼び出し＋発火安定性が揃った）。再導入は設計変更につき本検証のスコープ外＝follow-up Issue 化。証拠 payload は `hook-investigation-report.md` の 1.0.9 追記に保存。
