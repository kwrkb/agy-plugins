...
- **`hooks.json` の使用は非推奨（agy 1.0.8 制約）**: 現状の agy では、フックの標準入力に編集対象ファイルパスが含まれない（`toolCall: null`）、変数（`${extensionPath}` 等）が置換されずシェルエラーになる、PWD がワークスペースから外れる等の致命的な仕様のため、**ファイル連動の自動処理用途としては利用不可能**（実機検証済み）。処理の自動化はフックではなく明示的なコマンド（例: `/agy-plugin-kit:validate`）に頼る設計を標準とする。
- **`rules/` は使わない（agy 1.0.8 制約）**: プラグイン内 `rules/*.md`・`plugin.json` の `"rules": [...]`・グローバル `~/.gemini/rules/*.md` のいずれも、実機検証ではシステムプロンプトに**一切注入されなかった**（機能未実装/不具合）。プラグインからエージェントへ固有の知識・規約を渡すには、必ず `skills/<name>/SKILL.md`（呼び出し時ロード）として定義する。
