...
本リポジトリに含まれる `plugin.json` および `mcp_config.json` は、互換クライアントがこのMCPサーバーをロードするための構成ファイルです。
...
> 2. インストール完了後、`agy` CLI のパス解決の仕様により、MCPサーバーへの接続（プロセスの起動）に失敗することがあります。その場合は、インストール先の構成ファイル（例: `~/.gemini/config/plugins/gitlab/plugin.json` および `mcp_config.json` 等）を開き、`"command"` の値を `./mcpServers/...` のような相対パスから、**実行バイナリの完全な絶対パス** に直接書き換えてください。
