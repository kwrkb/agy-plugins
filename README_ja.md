...
Go 製プラグイン（`github` / `ast-grep` / `go-lsp` / `retro-status` / `settings-advisor` / `agy-plugin-kit` の validator）は **`linux/amd64` / `darwin/arm64`（Apple Silicon）/ `windows/amd64`** のネイティブバイナリを `bin/` に同梱しています。`bin/<name>` の OS 分岐 dispatcher が `uname` から `<name>-<goos>-<goarch>` を算出して exec します（Windows は agy が `.exe` を直接起動）。
...
各プラグインは公式 MCP 実装または CLI のラッパーであり、対象のライセンスに準拠します。
...
| retro-status | 自前 Go 実装 | MIT |
| settings-advisor | 自前 Go 実装 | MIT |
| agy-plugin-kit | 自前 Go 実装 | MIT |
